...
function init_network(params)
    -- input: table of length (num_past + num_future - 1) of table of length num_obj of {(bsize, obj_dim)}  -- just set it to 10
    -- the first element of the inner table is teh data, and the second element of the inner table is the mask

    -- "social" pooling is just a sum
    ...
    local hid_dim = max_obj*rnn_dim/2  -- TODO rename rnn_dim to hid_dim
    ...
    -- single timestep
    local input = nn.Identity()()  -- table of length num_obj of {(bsize, obj_dim)} 
    local nbrhd_masks = nn.Identity()()  -- table of length num_obj of tensor (bsize, num_obj, hid_dim)

    -- This takes in the input
    local object_core = nn.Sequential()
    if num_layers == 1 then
        object_core:add(nn.Linear(in_dim, hid_dim))
    else
        for i = 1, num_layers do -- TODO make sure this is comparable to encoder decoder architecture in terms of layers
            if i == 1 then 
                ...
                object_core:add(nn.Linear(hid_dim, hid_dim))
                object_core:add(nn.ReLU())
            end
        end
    end
    ...
    -- table of num_obj of {(bsize, hid_dim)}
    local object_encodings = object_cores(input)

    local reshaped_object_encodings = nn.Sequencer(nn.Reshape(mp.batch_size, 1, hid_dim))(object_encodings)

    -- tensor (bsize, num_obj, hid_dim)
    local object_encodings_tensor = nn.JoinTable(2)(reshaped_object_encodings)

    -- MAY NEED TO DO A PARALLEL TABLE HERE!

    local nbrhd_mask = nn.Identity()()  -- tensor (bsize, num_obj, hid_dim)
    local masked_object_encodings_tensor = nn.CMulTable(){nbrhd_mask,object_encodings_tensor}  -- tensor (bsize, num_obj, hid_dim)
    local reduced_masked_object_encodings_tensor = nn.Sum(2)(masked_object_encodings_tensor)  -- tensor (bsize, hid_dim)
    local reshaped_reduced_masked_object_encodings_tensor = nn.Reshape(mp.batch_size, 1, hid_dim)(reduced_masked_object_encodings_tensor)  -- tensor (bsize, 1, hid_dim)
    local masker_core = nn.gModule({nbrhd_mask,object_encodings_tensor},{reshaped_reduced_masked_object_encodings_tensor})
    local masker_sequencer = nn.Sequencer(masker_core)  -- outputs table of length num_obj of tensors (bsize, 1, hid_dim)
    local masker_sequencer_out = masker_sequencer(nbrhd_masks)
    ...
    local timestep_out = decoders(object_decoder_input)  -- table of length num_obj of (bsize, out_dim)

    local net = nn.gModule({input, nbrhd_masks},{timestep_out})
    assert(false, 'did you check table.insert bugs?')
