...
params = {model='ffobj', rnn_dim=100, input_dim=10, num_future=1, object_dim=10, layers=3, num_obj=4}
mp = {cuda=false}
...
function model.create(mp_, preload, model_path)
    ...
    if preload then
        ...
        self.network = init_clique_rnn_memory(self.mp)
        if self.mp.cuda then
            self.network:cuda()
            self.criterion:cuda()
